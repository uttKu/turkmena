...
exports.run = async (client, msg, args) => {
  ...
  if (!query) return msg.reply('Oyun adını giriniz').then(msg => msg.delete(5000));
  ...
  if (query === "minecraft" || query === "mc" || query === "Minecraft" || query === "MC" || query === "Mc" || query === "MİNECRAFT" || query === "MINECRAFT" ) {
    ...
    const embed = new Discord.RichEmbed()
			...
			return msg.reply(`Bir Hata Oluştu! \n**Hata:** \n\`${err.message}\``).then(msg => msg.delete(3000));
    }
    return;
  }
  ...
  try {
			...
			if (!id) return msg.reply('Bulunamadı').then(msg => msg.delete(3000));
