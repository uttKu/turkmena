...
const app = express();
const ms = require("ms");
const emojiChannelID = '730808617963356182';
...
client.on('ready', async () => {
  try {
    ...
    if (!channel) return console.error('Gecerli olmayan kanal');
...
client.on("messageDelete", async msg => {
  ...
  let embed = new Discord.RichEmbed()
    ...
    .setColor("#fc3c3c")
    .addField("Yazar:", msg.author.tag, true)
    .addField("Kanal:", msg.channel, true)
    .addField("Mesaj:", msg.content)
    .addField("Silen:", entry.executor, true)
    .addField("Sebep:", entry.reason || "Belirtilmemiş", true)
    .setFooter(`Mesaj İd: ${msg.id} | Yazar İd: ${msg.author.id}`);
  let channel = msg.guild.channels.find(x => x.name === 'msg-log');
  channel.send({embed});
});
...
client.on('error', e => {
 ...
 client.on('message', async msg => {
  ...
  else ozelkomutYazi = prefix + ''+ ozelkomut +''
...
client.on("message", msg => {
  ...
  if (kufurEngel[msg.guild.id].küfürEngel === 'kapali') return;
    if (kufurEngel[msg.guild.id].küfürEngel=== 'acik') {
      const kufur = ["o-ç", "a-m-k", "mk", "amk", "aq", "orospu", "oruspu", "oç", "sikerim", "yarrak", "piç", "amq", "sik", "amcık", "çocu", "sex", "seks", "amına", "orospu çocuğu", "sg", "siktir git","orospu çocu"];
  if (kufur.some(word => msg.content.toLowerCase().includes(word)) ) {
    if (!msg.member.hasPermission("ADMINISTRATOR")) {
      msg.delete()
       msg.reply("Bu sunucuda küfürler **TURKMENA Bot** tarafından engellenmektedir! Küfür etmene izin vermeyeceğim!").then(message => message.delete(20000));
    }
}
    }
});
...
const yourID = "723000129535213609";
const setupCMD = "r!emojilidoğrulama"
let initialMessage = `Türkmena Towny Discord sunucumuza hoşgeldiniz, şuanda ziyaretçisiniz, sunucumuza oyuncu olarak katılmak için aşağıdaki emojiye yanıt verebilirsiniz, yanıt vererek tüm kurallarımızı ve yaptırımlarını kabul etmiş olursunuz.`;
const roles = ["Oyuncu"];
const reactions = ["✅"];
const botToken = "NzMzNDUxMzA4MjExNzY1Mjcw.XxDVwg.e6ic5XQ7rDFr_Gfxf9M7UHsQHTM";
...
function generateMessages(){
    ...
    for (let role of roles) messages.push(` **"${role}"** Yetkisini Almak İçin Alttaki Emojiye Tıkla!`); 
    return messages;
}
...
client.on("guildMemberAdd", async member => {
   const fs = require('fs');
    let gkanal = JSON.parse(fs.readFileSync("./glog.json", "utf8"));
...
client.on('message', msg => {
  ...
  if (msg.channel.name== '🔧丨destek') { 
    ...
    if(msg.guild.channels.find('name', 'talepler')) {
      ...
      .addField(`» Talep Eden:`, `<@${msg.author.id}>`)
      .addField(`» Talep Detayı:`, `${msg.content}`, true)
